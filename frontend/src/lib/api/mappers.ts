...
function isStoreOpen(open?: string, close?: string, metadata?: any): boolean {
  const now = new Date();
  const dayNames = ['Sunday', 'Monday', 'Tuesday', 'Wednesday', 'Thursday', 'Friday', 'Saturday'];
  const todayName = dayNames[now.getDay()];

  if (metadata?.hours && Array.isArray(metadata.hours)) {
    const todayHours = metadata.hours.find((h: any) => h.day.toLowerCase() === todayName.toLowerCase());
    if (todayHours) {
