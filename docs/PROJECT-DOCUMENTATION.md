...
FCM push (gap #10) layers on the same notification hooks: `utils/fcm.ts` sends a message to every registered device of a recipient (fire-and-forget, invalid tokens pruned), the frontend registers its token on login via `/api/devices`, and `public/firebase-messaging-sw.js` shows background notifications. Push is optional — without `FIREBASE_*` env vars it is silently disabled and the socket/in-app/polling channels carry the load.
...
| 10 | **Push notifications channel is a placeholder** (`channel: ['push']`, nothing sends). **Done** — FCM integration end to end: a `DeviceToken` registry (migration `20260913000000_device_tokens`) plus `POST/GET/DELETE /api/devices` (identity always from the JWT) lets each signed-in lane register its FCM token; `utils/fcm.ts` (`sendPush`/`enqueuePush`) fans notifications out to every device of a recipient via multicast and prunes dead tokens, and every existing notify hook (orders, seller, delivery + assignment, payments, payouts, admin sellers/support/users) now calls it. The frontend mints/registers the token on login and revokes it on logout (`PushNotificationManager` + `public/firebase-messaging-sw.js`). Push is a hard-optional channel: without `FIREBASE_*` env vars (or without permission) everything degrades to in-app + socket + polling. Guarded by `check-push-notifications.ts` + `fcm-config.test.ts`. | Customers miss order updates off-site. | FCM integration: register device tokens per user, send on the existing notification hooks. |
