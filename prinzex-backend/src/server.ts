 ...
 * MongoDB and Redis -> start HTTP listener. If ANY database connection
 * fails the process exits with a non-zero code — a half-connected service
 * is worse than a fast crash.
 */
async function bootstrap(): Promise<void> {
  ...
  const server: Server = app.listen(env.PORT, () => {
    logger.info(`PrinZex API listening on http://localhost:${env.PORT}`, {
      port: env.PORT,
...
function registerShutdown(server: Server): void {
  const shutdown = (signal: string): void => {
    ...
    server.close(() => {
      void (async () => {
        await Promise.allSettled([disconnectDatabase(), disconnectMongo()]);
        disconnectRedis();
