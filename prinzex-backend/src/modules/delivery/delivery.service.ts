...
export async function getProfile(deliveryBoyId: string) {
  const boy = await prisma.deliveryBoy.findUnique({
    ...
    include: { bankDetails: true, documents: true, pincodes: { include: { registry: { include: { city: { select: { name: true } } } } } } },
  });
  if (!boy) {
  ...
  return {
    ...
    coverage: boy.pincodes.map((row) => ({
      ...
      city: row.registry.city?.name ?? row.registry.citySlug,
...
export async function adminGetDeliveryBoy(id: string) {
  const boy = await prisma.deliveryBoy.findUnique({
    ...
    include: {
      bankDetails: true,
      documents: true,
      pincodes: { include: { registry: { include: { city: { select: { name: true } } } } } },
      deliveries: {
        orderBy: { createdAt: 'desc' },
        take: 10,
