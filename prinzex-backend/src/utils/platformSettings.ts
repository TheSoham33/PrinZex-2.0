...
export interface PlatformSettingsValues {
  ...
  /** Disputes: hours the fulfilling seller gets to accept/reject a customer
   *  complaint before it auto-escalates to admin (final tier). */
...
export const SETTING_BOUNDS = {
  ...
  complaintResponseWindowHours: { min: 1, max: 168, maxDecimals: 0 },
} as const;
...
export const getComplaintResponseWindowHours = async (): Promise<number> =>
  (await getPlatformSettingsValues()).complaintResponseWindowHours;

/** Canonical slug for a free-form city name ("Kolkata" → "kolkata"). */
...
/**
 * City-aware platform values: the global Settings → Platform numbers, with
 * the city registry's optional deliveryFees / deliveryEtaHours overrides
 * layered on top per speed (a missing/invalid speed falls back to the
 * platform default — overrides only need to carry what diverges).
 */
export async function getPlatformSettingsValuesForCity(
  cityName: string | null | undefined,
): Promise<PlatformSettingsValues> {
  const base = await getPlatformSettingsValues();
  const slug = cityName ? citySlugFromName(cityName) : '';
  if (!slug) return base;

  let row: { deliveryFees: unknown; deliveryEtaHours: unknown } | null = null;
  try {
    const { prisma } = await import('../config/database');
    row = await prisma.city.findUnique({ where: { slug }, select: { deliveryFees: true, deliveryEtaHours: true } });
  } catch {
    // Registry unreadable — platform defaults apply (same fail-open policy).
  }
  if (!row) return base;

  const feesSource = (row.deliveryFees ?? {}) as Record<string, unknown>;
  const etaSource = (row.deliveryEtaHours ?? {}) as Record<string, unknown>;
  const hasFeeOverride = row.deliveryFees != null && typeof row.deliveryFees === 'object';
  const hasEtaOverride = row.deliveryEtaHours != null && typeof row.deliveryEtaHours === 'object';

  return {
    ...base,
    deliveryFees: hasFeeOverride
      ? mergeSpeedMap(feesSource, base.deliveryFees, SETTING_BOUNDS.deliveryFee)
      : base.deliveryFees,
    deliveryEtaHours: hasEtaOverride
      ? mergeSpeedMap(etaSource, base.deliveryEtaHours, SETTING_BOUNDS.deliveryEtaHours)
      : base.deliveryEtaHours,
  };
}
